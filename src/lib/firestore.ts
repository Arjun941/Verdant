...
export async function addTransaction(userId: string, transaction: Omit<Transaction, 'id'>) {
  try {
    ...
    await addDoc(transactionsCol, {
      ...transaction,
      date: new Date(transaction.date), // Firestore needs proper Date objects for sorting
...
export async function getTransactions(userId: string): Promise<Transaction[]> {
  try {
    ...
    return querySnapshot.docs.map(doc => {
      const data = doc.data();
      // Turn Firestore dates back into strings for the frontend
...
// Manage financial insights generated by Verdant AI
...
export async function addInsight(userId: string, insight: Omit<Insight, 'id'>): Promise<string> {
  try {
    ...
    const docRef = await addDoc(insightsCol, {
      ...insight,
      createdAt: new Date(insight.createdAt), // Firestore needs Date objects for timestamps
    });
    return docRef.id;
  } catch (error) {
